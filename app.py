...
# ── Inference ─────────────────────────────────────────────────────────────────
@torch.no_grad()
def predict(model, pipeline, raw_df: pd.DataFrame, seq_len: int = 60):
    feat_df = add_technical_indicators(raw_df.copy())
    ...
    if len(X) < seq_len:
        return None, None, f"Need ≥{seq_len} rows after feature engineering, got {len(X)}."
    ...
    return prob_up, attn_weights.squeeze(0).numpy(), None
...
# ── Charts ────────────────────────────────────────────────────────────────────
def make_price_chart(df: pd.DataFrame, days: int = 120) -> go.Figure:
    ...
    fig.add_trace(go.Scatter(
        ...
        name="EMA 21", line=dict(color="#f59e0b", width=1, dash="dot"),
    ))
    fig.add_trace(go.Scatter(
        x=df_plot.index, y=df_plot["EMA50"],
        name="EMA 50", line=dict(color="#ec4899", width=1, dash="dot"),
    ))
    fig.update_layout(
...
def make_attention_chart(attn: np.ndarray, seq_len: int = 60) -> go.Figure:
    labels = [f"t-{seq_len - i - 1}" for i in range(seq_len)]
    # Only show every 5th label for readability
    tick_vals = list(range(0, seq_len, 5))
    tick_text = [labels[i] for i in tick_vals]
...
def main():
    ...
    with col_title:
        st.markdown("""
        <h1 style='font-family: IBM Plex Mono; font-size: 26px; font-weight: 600;
                   color: #f1f5f9; margin-bottom: 4px;'>
            S&P 500 Direction Predictor
        </h1>
        <p style='font-family: IBM Plex Mono; font-size: 12px; color: #475569;
                  letter-spacing: 1px;'>
            BiLSTM + Temporal Attention &nbsp;·&nbsp; 44 Technical Indicators
            &nbsp;·&nbsp; 1986–2024 Training Data
        </p>
        """, unsafe_allow_html=True)
    with col_badge:
        st.markdown("""
    ...
    with st.sidebar:
        st.markdown("""
        <p style='font-family: IBM Plex Mono; font-size: 11px; letter-spacing: 2px;
        ...
        st.divider()
        st.markdown("""
        <p style='font-family: IBM Plex Mono; font-size: 10px; color: #334155;
                  letter-spacing: 1px; text-transform: uppercase; margin-bottom: 8px;'>
            About
        </p>
        <p style='font-size: 12px; color: #475569; line-height: 1.6;'>
            Bidirectional LSTM with soft temporal attention.
            Trained on 9,827 daily bars of ^GSPC.
            Outputs P(close₊₁ > close₀).
        </p>
        <p style='font-size: 11px; color: #334155; margin-top: 12px;'>
            Shivam Tiwari · 065104
        </p>
        """, unsafe_allow_html=True)
    ...
    if run_btn or st.session_state.get("has_prediction"):
        ...
        if model is None:
            ...
            if inf_err:
                ...
                with pred_col:
                    st.markdown(f"""
                    <div class='{dir_class}'>
                        <div class='direction-label' style='color:{dir_color}'>
                            PREDICTED DIRECTION
                        </div>
                        <div class='direction-arrow' style='color:{dir_color}'>
                            {dir_arrow}
                        </div>
                        <div class='direction-prob' style='color:{dir_color}'>
                            {direction}
                        </div>
                ...
                if show_attn and attn is not None:
                    st.markdown("<div class='section-header'>TEMPORAL ATTENTION WEIGHTS</div>",
                                unsafe_allow_html=True)
                    st.caption(
                        "Which of the last 60 trading days the model weighted most heavily. "
                ...
                if show_indic:
                    ...
                    ind_metrics = [
                        (ic1, "RSI 14",     f"{latest.get('RSI_14', 0)*100:.1f}",
                         "Overbought >70, Oversold <30"),
                        (ic2, "MACD",       f"{latest.get('MACD_norm', 0)*100:.3f}%",
                         "Normalised vs price"),
                        (ic3, "BB Position",f"{latest.get('BBP', 0)*100:.1f}%",
                         "0%=lower band, 100%=upper"),
                        (ic4, "Vol Ratio",  f"{latest.get('VOL_ratio', 0):.2f}×",
                         "vs 20-day avg volume"),
                        (ic5, "Stoch %K",   f"{latest.get('STOCHk', 0)*100:.1f}",
                         "14-day stochastic"),
                    ]
                    for col, lbl, val, sub in ind_metrics:
                        with col:
                            st.markdown(f"""
                ...
                <div class='disclaimer'>
                    ...
                    The model achieved ~55% directional accuracy on a held-out test set —
                    marginally above chance. Do not make investment decisions based on
                    these predictions.
                </div>
                """, unsafe_allow_html=True)
        ...
        <div style='text-align:center; padding: 60px 0; color: #334155;'>
            ...
            <p style='font-family: IBM Plex Mono; font-size: 13px; letter-spacing: 2px;'>
                PRESS RUN PREDICTION TO BEGIN
            </p>
        </div>
        """, unsafe_allow_html=True)
    else:
        st.info("📁 Add your model artifacts to the `model/` directory to enable predictions.\n\n"
